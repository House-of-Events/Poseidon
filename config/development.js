  ...
  requiredKeys.forEach(key => {
    try {
      ...
      if (temporaryKeys.includes(key)) {
        ...
        logger.warn(`Warning: ${key} is not set in Chamber for ${service}`);
      } else {
...
export default {
  ...
  AWS_ACCESS_KEY_ID:
    appSecrets.aws_access_key_id || process.env.AWS_ACCESS_KEY_ID,
  AWS_SECRET_ACCESS_KEY:
    appSecrets.aws_secret_access_key || process.env.AWS_SECRET_ACCESS_KEY,
  ...
  // S3 Configuration
  S3_BUCKET_NAME:
    appSecrets.s3_bucket_name || 'dev-fixtures-daily-details-bucket',
